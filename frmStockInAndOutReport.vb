...
Public Class frmStockInAndOutReport
    ...
    Private Sub btnGetData_Click(sender As System.Object, e As System.EventArgs) Handles btnStockIn.Click
        Try
            ...
            con.Open()
            cmd = New SqlCommand("SELECT RTRIM(Product.ProductCode),RTRIM(ProductName),SellingPrice,Discount,VAT,Qty from Temp_Stock,Product where Product.PID=Temp_Stock.ProductID and Qty > 0 order by PID", con)
    ...
    Private Sub Button1_Click(sender As System.Object, e As System.EventArgs) Handles btnStockOut.Click
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("SELECT (P.ProductCode),(P.ProductName),II.Qty,P.SellingPrice,P.Discount,P.VAT from Temp_Stock S,Product P,Invoice_Product II where P.PID=S.ProductID AND P.PID =  II.ProductID and II.Qty > 0 order by PID", con)
            adp = New SqlDataAdapter(cmd)
            dtable = New DataTable()
            adp.Fill(dtable)
            con.Close()
