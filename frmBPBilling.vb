...
Public Class frmBPBilling
    ...
    Sub auto()
        Try
            ...
            txtInvoiceNo.Text = "INVBP-" + GenerateID()
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.[Error])
        End Try
    End Sub
    Private Sub btnSelect_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSelect.Click
        frmCustomerRecord2.lblSet.Text = "Manufactured"
    ...
    Private Sub Button2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSelectionInv.Click
        'frmCurrentStock.lblSet.Text = "Billing"
        'frmCurrentStock.Reset()
        'frmCurrentStock.ShowDialog()
        frmProductRecord.lblSet.Text = "Manufactured"
        frmProductRecord.Reset()
        frmProductRecord.ShowDialog()
    End Sub
    Private Sub StockValidation()
        Dim TableQty As Integer
        Dim opQty As Integer = 0
        opQty = Val(txtQty.Text)
        con = New SqlConnection(cs)
        con.Open()
        Dim cb2 As String = "SELECT Qty FROM build_Stock where OurProductID=@d1"
        ...
        If rdr.HasRows Then
            ...
            If opQty > TableQty Then
                MessageBox.Show("Sorry you have no Stock available to sell this item! ", "Stock Empty", MessageBoxButtons.OK, MessageBoxIcon.Information)
                SetNull()
            End If
        End If
        rdr.Close()
    ...
    Sub Print()
        Try
            If txtCustomerType.Text <> "Non Regular" Then
                ...
                Dim rpt As New rptBPBilling 'The report you created.
    ...
    Private Sub btnListReset1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnListReset1.Click
        Clear1()
    End Sub
    ...
    Private Sub btnListUpdate1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnListUpdate1.Click
        Try
            If DataGridView1.Rows.Count = 0 Then
                MessageBox.Show("sorry no product added to cart", "", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Exit Sub
    ...
    Private Sub btnSaveNew_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSaveNew.Click
        ...
        Try
            ...
            For Each row As DataGridViewRow In DataGridView1.Rows
                Dim con As New SqlConnection(cs)
                con.Open()
                Dim cmd As New SqlCommand("SELECT Qty from build_Stock where OurProductID=@d1", con)
                cmd.Parameters.AddWithValue("@d1", row.Cells(12).Value.ToString())
                Dim da As New SqlDataAdapter(cmd)
                Dim ds As DataSet = New DataSet()
                da.Fill(ds)
            ...
            For Each row As DataGridViewRow In DataGridView1.Rows
                If Not row.IsNewRow Then
                    con = New SqlConnection(cs)
                    con.Open()
                    Dim cb4 As String = "update build_Stock set qty = qty - (" & row.Cells(5).Value & ") where OurProductID=@d1"
                    cmd = New SqlCommand(cb4)
                    cmd.Connection = con
                    cmd.Parameters.AddWithValue("@d1", row.Cells(12).Value)
                    cmd.ExecuteNonQuery()
    ...
    Private Sub btnGetDataNew_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGetDataNew.Click
        frmBpBillingRecord.Reset()
        frmBpBillingRecord.ShowDialog()
    End Sub
    ...
    Private Sub btnSalesmanSelect_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSalesmanSelect.Click
        frmSalesmanRecord.lblSet.Text = "Manufactured"
