...
Public Class frmLogs
    ...
    Private Sub btnGetData_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGetData.Click
        Try
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("SELECT RTRIM(UserID),Date,RTRIM(Operation) from logs where Date >=@datefrom and Date < @dateto order by Date", con)
            cmd.Parameters.Add("@datefrom", SqlDbType.DateTime, 30, "Date").Value = dtpDateFrom.Value.Date.AddDays(0)
            cmd.Parameters.Add("@dateto", SqlDbType.DateTime, 30, "Date").Value = dtpDateTo.Value.Date.AddDays(1)
            rdr = cmd.ExecuteReader(CommandBehavior.CloseConnection)
            dgw.Rows.Clear()
            While (rdr.Read() = True)
                dgw.Rows.Add(rdr(0), rdr(1), rdr(2))
